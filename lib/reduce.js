...
const getStatsReduce = (results) => {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (results[i].size) reduce.size += results[i].size;
        if (results[i].records) reduce.records += results[i].records;
        ...
        if (i === 0) {
            ...
            if (results[i].records < reduce.min) reduce.min = results[i].records;
            if (results[i].records > reduce.max) reduce.max = results[i].records;
