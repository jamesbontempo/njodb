...
const insertStoreData = function(store, data, lockoptions, debug) {
...
const selectStoreData = function(store, selecter, lockoptions, debug) {
    return new Promise ((resolve, reject) => {
        const start = Date.now();
        var line = 0;
        var selected = 0;
...
const deleteStoreData = function(store, selecter, tempstore, lockoptions, debug) {
    return new Promise((resolve, reject) => {
        const start = Date.now();
        var line = 0;
        var deleted = 0;
...
const updateStoreData = function(store, selecter, updater, tempstore, lockoptions, debug) {
    return new Promise((resolve, reject) => {
        const start = Date.now();
        var line = 0;
        var updated = 0;
...
const aggregateStoreData = function(store, selecter, indexer, field, lockoptions, debug) {
    return new Promise((resolve, reject) => {
        const start = Date.now();
        var line = 0;
        var aggregate = {};
        if (fs.existsSync(store)) {
            lockfile.lock(store, lockoptions).then((release) => {
                ...
                readstream.on("line", (record) => {
                    ...
                    try {
                        record = JSON.parse(record);
                        if (selecter(record) && record[field] !== undefined) {
                            const index = indexer(record);
                            if (aggregate[index]) {
                                if (record[field] < aggregate[index]["min"]) aggregate[index]["min"] = record[field];
                                if (record[field] > aggregate[index]["max"]) aggregate[index]["max"] = record[field];
                                aggregate[index]["count"]++;
                                if (typeof record[field] === "number") {
                                    const delta1 = record[field] - aggregate[index]["mean"];
                                    aggregate[index]["sum"] += record[field];
                                    aggregate[index]["mean"] += delta1 / aggregate[index]["count"];
                                    const delta2 = record[field] - aggregate[index]["mean"];
                                    aggregate[index]["m2"] += delta1 * delta2;
                                }
                            } else {
                                aggregate[index] = {
                                    field: field,
                                    ...
                                    max: record[field],
                                    count: 1,
                                    sum: (typeof record[field] === "number") ? record[field] : undefined,
                                    mean: (typeof record[field] === "number") ? record[field] : undefined,
                                    m2: (typeof record[field] === "number") ? 0 : undefined
                                };
                            }
                        }
                    } catch (error) {
                ...
                readstream.on("close", () => {
                    release()
                        .then(() => {
                            ...
                            resolve({store: store, aggregate: aggregate, start: start, end: Date.now()})
                        });
                });
            }).catch((error) => {
                reject (error);
...
const indexStoreData = function(store, field, lockoptions, debug) {
    return new Promise((resolve, reject) => {
        const start = Date.now();
        var line = 0;
        var index = {};
