...
const insertFileData = async (file, datapath, storenames) => {
    ...
    // ADD TRY; maybe separate into individual inserts for each store?
...
const selectStoreData = async (store, match, project) => {
    ...
    try {
        const handlerResults = await new Promise((resolve, reject) => {
            ...
            reader.on("line", record => { try { handler.next(record); } catch(e) { reject(e); } });
            reader.on("close", () => resolve(handler.return()));
            reader.on("error", error => reject(error));
        });
    ...
    try {
        const handlerResults = await new Promise ((resolve, reject) => {
            ...
            writer.on("open", () => {
                ...
                reader.on("line", record => { try { handler.next(record, writer); } catch(e) { reject(e); } });
                reader.on("close", () => { writer.end(); resolve(handler.return()); });
                reader.on("error", error => reject(error));
            });
