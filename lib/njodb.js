            ...
            lockfile.lock(store, lockoptions).then((release) => {
                ...
                readstream.on("line", (record) => {
                    ...
                    try {
                        ...
                        } else if (error.message !== "Missing index") {
                            reject(error);
                        }
                    }
                });
...
const statsStoreData = async (store, lockoptions, debug) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (fs.existsSync(store)) {
            ...
            lockfile.lock(store, lockoptions).then((release) => {
                ...
                if (debug) console.log([store, "statsStoreData", "lock", Date.now() - start].join("\t"));
                ...
                try {
                    ...
                    readstream.on("close", () => {
                        release()
                            .then(() => {
                                if (debug) console.log([store, "statsStoreData", "done", Date.now() - start].join("\t"));

                                fs.stat(store, (error, stats) => {
                                    if (error) reject(error);
...
const deleteFile = async (filepath, lockoptions, debug) => {
    return new Promise((resolve, reject) => {
        const start = Date.now();

        if (fs.existsSync(filepath)) {
            lockfile.lock(filepath, lockoptions).then((release) => {
                if (debug) console.log([filepath, "deleteFile", "lock", Date.now() - start].join("\t"));

                fs.unlink(filepath, (error) => {
                    if (error) reject(error);
                    release()
                        .then(() => {
                            if (debug) console.log([filepath, "deleteFile", "done", Date.now() - start].join("\t"));
